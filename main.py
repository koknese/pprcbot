...
async def on_ready():
    ...
    await bot.load_extension("cogs.exp")
    print("EXP cog loaded")
    #await bot.load_extension("cogs.baseloading")
    #print("Baseloading cog loaded")

    #await bot.load_extension("cogs.raiderwatch")
    #print("Raiderwatch cog loaded")

    #await bot.load_extension("cogs.quickcheck")
    #print("Quickcheck cog loaded")

    await tree.sync(guild=discord.Object(id=server_id))  # Sync the commands after loading the cog
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name =f"over Phoenix employees"))
